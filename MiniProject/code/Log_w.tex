...
\begin{document}
	...
	\begin{figure}[h]
		...
		\caption{Barplot showing the number of ``best model" identification under AIC model-selection methods with ``
			...
			, p = 
			%% insert_num_here
		}\label{barPT} %% numbers need automation later
	\end{figure}
	Among all \pms, only \fve\ was identified being the best model (Fig.\ref{barPT}).  Polynomials were being the ``best-fit" model for 
	%% insert_num_here
	\% cases while \fve\ alone was identified for 
	%% insert_num_here
	\% cases (out of 
	%%insert_num_here
	 cases, all percentages rounded to nearest integer).
