...
pdf("../results/PP_Regress.pdf");ggplot(data = oo,aes(x=oo$Prey.mass,y=oo$Predator.mass,colour=oo$Predator.lifestage))+
  ...
  scale_x_continuous(labels=function(x) format(round(x), scientific = TRUE), limits = c(1e-7,1e1), trans = "log2")+scale_y_continuous(labels=function(x) format(round(x), scientific = TRUE), limits = c(1e-6,1e6), trans = "log2");dev.off()
