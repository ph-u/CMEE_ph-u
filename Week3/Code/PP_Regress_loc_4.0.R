...
## input
oo<-read.csv("../Data/EcolArchives-E089-51-D1.csv", header = T, stringsAsFactors=T)
for(i in 1:dim(oo)[1]){
...
colnames(oo.0)<-c("FeedingType", "LifeStageCategory", "Location", "slopeLinear", "interceptLinear","R2", "F-statistics", "p-val", "PredatorMassUniqueness", "PreyMassUniqueness")
  ...
  ## det factors
a.0<-levels(oo$Type.of.feeding.interaction) ## R3.x uses factor as default; R4.x uses character as default
